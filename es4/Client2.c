...
int main(int argc, char *argv[]){
	...
	char ok, buff[DIM_BUFF], nome_dir[LENGTH_DIR_NAME];
	struct hostent *host;
	...
	/* Da specifica, voglio che uno stesso cliente mantenga un’unica connessione 
	   per tutte le possibili invocazioni del direttorio: 
	   quindi se un cliente chiede più direttori di fila, 
	   il server deve mantienere la stessa connessione per servire i dir che 
	   quel cliente ha richiesto di fila!!!)
	*/
	
	/*  il ciclo dovrà avvenire dopo aver creato la connessione, 
	la quale dovrà rimanere on per tutto il tempo necessario: 
	voglio poter trattare più dir usando la stessa connessione.
	ORDINE:
		-socket
		-connect
		-inizio ciclo:
	...
	*/
	...
	while (gets(nome_dir) != NULL){ //lo facciamo così?
	
	/*Il client invia al server la richiesta con quel nome di direttorio: */
